...
@Service
public class HotelUploadFileService implements InitializingBean{
	@Autowired
    ...
    public void afterPropertiesSet() {
       fileDir = "C:\\Fullstack\\WorkSpace\\SpringMVC\\touring\\src\\main\\webapp\\resources\\img\\hotel";
    ...
    public List<HotelFileVo> upload(List<MultipartFile> multiList, int hno) {
        List<HotelFileVo> fileDataList = new ArrayList<>();	
        try {
            for (MultipartFile file : multiList) {
                ...
                HotelFileVo fileData = new HotelFileVo();
                fileData.setHno(hno);
                System.out.println("파일 tno: " + hno);// 관광지 번호
                fileData.setHoriginal(oriFileName);   // 원본 파일명
                fileData.setHfilename(uniqueName + ext); // 저장 파일명
                fileData.setSize(saveFile.length());   // 파일 크기

                System.out.println(fileData + " 업로드 완료");
                fileDataList.add(fileData);
                System.out.println(fileData.getHno());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
